...
//!
//! Januas fork (S5.5d.5): only `src/shader.wgsl` is modified — see
//! `UPSTREAM_NOTES.md` for the diff. The blanket clippy + lint allow below
//! preserves upstream's lint baseline because chasing pedantic/nursery
//! findings across 1.6 KLoC of vendored code would balloon the re-sync diff.
#![allow(
    missing_docs,
    unsafe_code,
    clippy::all,
