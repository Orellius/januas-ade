...
//
// Linear sRGB blending makes light-on-dark text appear too thin because AA
// fringe pixels blend perceptually darker than expected. Dark-on-light text
// has the opposite problem — it already looks heavier than its geometric
// coverage. To compensate, we compute the text color's brightness (k) and
// boost the glyph alpha through enhance_contrast(). Brighter text gets a
// stronger boost; dark text is left unchanged.
//
...
// Lifted verbatim from Warp's MIT-licensed glyph_shader.wgsl, which the
// Warp team already adapted from MS Terminal.
...
fn fs_main(in_frag: VertexOutput) -> @location(0) vec4<f32> {
    switch in_frag.content_type {
        ...
        case 1u: {
            // Sample the mask channel — geometric coverage of the rasterized
            // glyph at this fragment. Boost it via the DirectWrite-style
            // contrast curve evaluated at the text color's brightness, then
            // multiply by the run's alpha. Equivalent to upstream when k=0
            // (pure black text), strictly brighter as k → 1.
